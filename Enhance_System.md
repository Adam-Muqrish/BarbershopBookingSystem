...
- **Change made:** In `static/resources/js/script.js`, extended the shared `[data-dropdown]` handler (added in 2.2) with a document-level click handler that, when a link inside any open dropdown menu (Appointment **and** Login) is clicked, hides that menu and resets `aria-expanded` _before_ the browser navigates. Verified with `node --check` (JS syntax OK). ✅
